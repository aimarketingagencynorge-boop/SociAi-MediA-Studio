...
import { X, Sparkles, Download, CheckCircle, Terminal, AlertCircle, Key, Camera, Layout } from 'lucide-react';
...
export const GenerationModal: React.FC<GenerationModalProps> = ({ type, prompt, lang, onClose, onSuccess, brandContext }) => {
  ...
  const statuses = [ t.genStatusInit, t.genStatusNeural, t.genStatusSynthesis, t.genStatusFinal ];
  ...
  const handleStartGeneration = async (skipKeyCheck = false) => {
    ...
    setNeedsKey(false);
    setError(null);
    setLogs([`[SYS] Initiating ${type} engine...`]);
    setProgress(5);
        ...
        <div className="p-8 space-y-8">
          {!isGenerating && !isDone && !error && !needsKey ? (
            ...
            <div className="text-center space-y-6 animate-fadeIn">
               ...
               <button onClick={onClose} className="w-full py-4 bg-cyber-purple rounded-xl font-bold uppercase">CLOSE</button>
            </div>
          )}
        </div>
      </div>
